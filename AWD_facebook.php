...
Class AWD_facebook extends AHWEBDEV_wpplugin{
	...
	public function discover_content(){
		...
		if($_POST['submit_adform']){
			if (!wp_verify_nonce($this->plugin_slug.'_AWD_ads_contact',$this->plugin_option_pref.'_nonce_AWD_ads_contact_field') && count($tab_error) == 0 ){
				...
				$message .= '<div id="message" class="updated fadeOnload"><p>'.__('Message sent',$this->plugin_text_domain).'</p></div>';
			}else{
		...
		<div id="AWD_adform" class="uiForm">
			<h2 id="custom_pub" style="margin:0;">'.sprintf(__('Looking for a web developer ? %s Contact a Wordpress Master',$this->plugin_text_domain),'<br />').'</h2>
			'.$message.'
			<form method="post">
				<table>
					<tr class="dataRow">
				...
				'.wp_nonce_field($this->plugin_slug.'_AWD_ads_contact',$this->plugin_option_pref.'_nonce_AWD_ads_contact_field',$echo=false).'
				<br /><input type="submit" name="submit_adform" value="'.__('Send',$this->plugin_text_domain).'"/>
			</form>
		</div>
		<br />';
		echo '<a href="http://www.ahwebdev.fr/plugins/facebook-awd-seo-comments.html" target="_blank"><img src="'.$this->plugin_url_images.'facebook-awd-seo-comments-promo.png" alt="SEO comments" width="258" height="105" border="0" /></a>';
	}
	/**
	* Admin content
	*/
	...
	*/
	public function support_content(){
		?>
		<h2><?php _e("Bug Tracker Page",$this->plugin_text_domain); ?></h2>
		<iframe src="http://trac.ahwebdev.fr/projects/facebook-awd" width="100%" height="600" scrolling="auto" frameborder="0"></iframe>
		<?php
	}
	...
	*/
	public function unsync_fbaccount(){
		if($_GET['action']=='unsync_fb'){
			update_user_meta($this->current_user->ID,'fb_email','');
			update_user_meta($this->current_user->ID,'fb_uid','');
			update_user_meta($this->current_user->ID,'fb_user_infos','');
		}
	}
	/**
	...
	public function connect_footer(){
		...
		<script type="text/javascript">
            window.fbAsyncInit = function(){
                ...
                //TODO Ajaxify this part, to fetch connection to php sdk
                ...
                FB.getLoginStatus(function(response) {
					if (response.status === 'connected') {
						//TODO Ajaxify this part to knwo if user is logged to wordpress.
	...
	public function get_the_open_graph_tags($options=array()){
		if(!empty($options)){
			foreach($options as $tag=>$tag_value){
				...
				$tag = str_replace(array(":mp4","_mp4",":html","_html"),array(""),$tag);
 			...
 			if($custom_post[$this->plugin_option_pref.'ogtags_disable'][0] == 0){
				...
				if($this->plugin_option[$prefix_option.'disable'] == 0 && $this->plugin_option[$prefix_option.'disable'] != ''){
					...
					//else use general settings
						...
						$video_mp4 = $this->plugin_option[$prefix_option.'video:mp4'];
						$video_html = $this->plugin_option[$prefix_option.'video:html'];
